...
class LGBTQNewsCollector:
    ...
    def __init__(self, newsapi_key: Optional[str] = None):
        ...
        self.rss_feeds = {
            #'google_atlanta': 'https://news.google.com/topics/CAAqHAgKIhZDQklTQ2pvSWJHOWpZV3hmZGpJb0FBUAE/sections/CAQiTkNCSVNORG9JYkc5allXeGZkakpDRUd4dlkyRnNYM1l5WDNObFkzUnBiMjV5Q2hJSUwyMHZNREV6ZVhGNkNnb0lMMjB2TURFemVYRW9BQSowCAAqLAgKIiZDQklTRmpvSWJHOWpZV3hmZGpKNkNnb0lMMjB2TURFemVYRW9BQVABUAE?hl=en-US&gl=US&ceid=US%3Aen',
            #'tonetoatl': 'https://www.tonetoatl.com/',
            #'atlantanewsfirst': 'https://www.atlantanewsfirst.com/arc/outboundfeeds/rss/',
            #'wsbtv': 'https://www.wsbtv.com/news/local/?format=rss',
            #'atlantadailyworld': 'https://atlantadailyworld.com/feed/',
            #'creativeloafing': 'https://creativeloafing.com/rss',
            #'atlantamagazine': 'https://www.atlantamagazine.com/feed/',
            #'atlantaeats': 'https://www.atlantaeats.com/feed/',
            #'atlantaga.gov': 'https://www.atlantaga.gov/Home/Components/RSS/',
            #'beltline': 'https://beltline.org/feed/',
            #'google_lgbtq_atlanta': #'https://news.google.com/rss/search?q=gay%20atlanta%20-matt%20-jazz&hl=en-US&gl=US&ceid=US%3Aen',
        }

        
        self.lgbtq_keywords = [
            'atlanta', 'Atlanta', 'ATL', 'Metro Atlanta', 'LGBT Atlanta','Downtown Atlanta', 'Midtown Atlanta', 'Buckhead', 'East Atlanta Village', 'West End Atlanta', 'Old Fourth Ward', 'Little Five Points', 'Decatur', 'Inman Park', 'Grant Park', 'Virginia Highland', 'BeltLine'
    ...
    def collect_from_newsapi(self, hours_back: int = 24) -> List[Article]:
        ...
        for keyword in self.lgbtq_keywords[:17]:    ###250722###
            self.logger.info(f"Fetching NewsAPI articles for keyword: {keyword}")
            ...
            try:
                ...
                if entry_count > 0:
                    successful_feeds.append(f"{keyword} ({entry_count} articles)")
                    self.logger.info(f"Successfully collected {entry_count} articles from keyword: {keyword}")
        ...
        if successful_feeds:
            self.logger.info(f"Successful NewsAPI collections: {', '.join(successful_feeds)}")
            ...
            function showStats() {
                ...
                document.getElementById('content').innerHTML = `
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 2fr; gap: 15px; align-items: center;">
                        <div><strong>🎯 Project:</strong></div><div>LGBTQ+ News Aggregator</div>
                        <div><strong>🔧 Status:</strong></div><div>Enhanced Production Ready</div>
                        <div><strong>📡 Sources:</strong></div><div>15 RSS feeds + NewsAPI</div>
                        <div><strong>🏷️ Keywords:</strong></div><div>17 LGBTQ+ focused terms</div>
                        <div><strong>🚫 Excluded:</strong></div><div>7 negative keywords filtered</div>
                        <div><strong>⚡ Features:</strong></div><div>Enhanced thumbnail extraction, caching, deduplication, timezone handling</div>
                        <div><strong>🖼️ Thumbnails:</strong></div><div>Multi-source extraction (RSS + URL scraping)</div>
                        <div><strong>🛡️ Resilience:</strong></div><div>Advanced error handling, retry logic, feed health monitoring</div>
                    </div>
